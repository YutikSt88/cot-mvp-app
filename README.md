...
**COT-MVP** — це система обробки та аналізу тижневих звітів Commitment of Traders (COT) від CFTC (Commodity Futures Trading Commission). Проект завантажує історичні дані COT, нормалізує їх та створює ML-ready датасети для аналізу позицій інституційних трейдерів.
...
- ✅ Реєстр всіх контрактів з raw snapshots
...
**Призначення:** Створення metrics dataset з позиційними метриками, WoW змінами, net exposure та rebalance decomposition
...
**Процес:**
1. Читає canonical_full dataset
2. Фільтрує по whitelist markets з `markets.yaml`
3. Розраховує метрики:
   - Поточні позиції (long/short/total для NC та COMM)
   - Position metrics: min/max/pos для ALL window та 5Y rolling window
   - WoW change: тижневі зміни (*_chg_1w)
   - Net metrics: nc_net, comm_net, spec_vs_hedge_net + *_chg_1w
   - Rebalance decomposition: gross/net_abs/rebalance + share
4. Виконує валідації (blocking): rows > 0, required columns, uniqueness, формули
...
**Position metrics:**
- Поточні: `nc_long`, `nc_short`, `nc_total`, `comm_long`, `comm_short`, `comm_total`
...
**Net metrics:**
- `nc_net`, `comm_net`, `spec_vs_hedge_net`
- `nc_net_chg_1w`, `comm_net_chg_1w`, `spec_vs_hedge_net_chg_1w`
...
**Важливо:** Новий ринок додається через contract_code (не через назву ринку). Поле `category` використовується для групування ринків.
...
6. **Separation of concerns:** Чітке розділення ingest → normalize → ml_backup → registry
...
python -m src.normalize.run_ml_backup --all-assets --log-level INFO
```
...
- `data/` - всі дані (raw, canonical, ml, registry)
- `reports/` - звіти (якщо генеруються; ігнорується git)
...
**Альтернативний спосіб (якщо є проблеми з імпортами):**
```powershell
$env:PYTHONPATH=(Get-Location).Path
python -m streamlit run src/app/app.py
```

Dashboard читає дані виключно з `data/compute/metrics_weekly.parquet` (немає обчислень в UI).

**Важливо:**
- ❌ Немає торгових сигналів
- ❌ Немає рекомендацій
